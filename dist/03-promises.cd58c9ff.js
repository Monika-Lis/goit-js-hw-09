...
form.addEventListener("submit", (event)=>{
    ...
    const delay1 = Number(event.target.delay.value);
    const step = Number(event.target.step.value);
    const amount = Number(event.target.amount.value);
    for(let i = 1; i <= amount; i++)createPromise(i, delay1 + step * (i - 1)).then(({ position , delay  })=>{
...
},{"notiflix":"fuMq1","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"fuMq1":[function(require,module,exports) {
var global = arguments[3];
